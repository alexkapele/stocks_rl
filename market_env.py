...
class MarketEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #Calculate inactive time for penalty (i.e. no position)
        ...
        if done==False:
            self.state = (self.features.iloc[self.window_counter+1,:], pos, abs(pos)-abs(pos_limit), (self.trading_window-1)-self.window_counter, self.stop_loss, self.take_profit)


        #Calculate average return for latest action based on the close price of the n following days (for reward function)
        #avg_return = (self.prices.iloc[self.window_counter:]['Close']).mean()/self.prices.iloc[self.window_counter]['Open']
        #n_days = min(3, self.trading_window - self.window_counter)
        #next_days_close_av_return = (self.prices.iloc[self.window_counter:self.window_counter+n_days+1]['Close']).mean()/self.prices.iloc[self.window_counter]['Open']
        #next_days_close_av_return = 1+trade_log[-1]*(next_days_close_av_return-1)
    ...
    def reset(self):
        ...
        self.action_log = [] #to keep log of all actions
        self.pos_log = [] #to keep log of all positions
        self.stop_loss_log = [] #to keep log of all stop_loss
